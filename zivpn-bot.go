...
const (
    ...
    // Interval untuk pengecekan status pembayaran QRIS
...
var pendingPayments = make(map[string]*PaymentInfo)
var paymentCheckerRunning = false
...
func main() {
    ...
    // --- PAYMENT CHECKER ---
    if !paymentCheckerRunning {
        go startPaymentChecker(bot, config.AdminID)
        paymentCheckerRunning = true
    }
    // ------------------------------------------------
...
func handleCallback(bot *tgbotapi.BotAPI, query *tgbotapi.CallbackQuery, adminID int64) {
    ...
    case query.Data == "menu_trial_15":
        showPaymentOptions(bot, query.Message.Chat.ID, 15) // 15 Hari dengan QRIS
    case query.Data == "menu_trial_30":
        showPaymentOptions(bot, query.Message.Chat.ID, 30) // 30 Hari dengan QRIS
    case query.Data == "menu_trial_60":
        showPaymentOptions(bot, query.Message.Chat.ID, 60) // 60 Hari dengan QRIS
    case query.Data == "menu_trial_90":
        showPaymentOptions(bot, query.Message.Chat.ID, 90) // 90 Hari dengan QRIS
    ...
    case query.Data == "menu_qris":
        showQrisInfo(bot, query.Message.Chat.ID)
    ...
    case strings.HasPrefix(query.Data, "pay_qris:"):
        parts := strings.Split(query.Data, ":")
        days, _ := strconv.Atoi(parts[1])
        initiateQrisPayment(bot, query.Message.Chat.ID, days)
    case strings.HasPrefix(query.Data, "check_payment:"):
        paymentID := strings.TrimPrefix(query.Data, "check_payment:")
...
func showMainMenu(bot *tgbotapi.BotAPI, chatID int64) {
    ...
    keyboard := tgbotapi.NewInlineKeyboardMarkup(
        ...
        tgbotapi.NewInlineKeyboardRow(
            tgbotapi.NewInlineKeyboardButtonData("💳 Info Pembayaran QRIS", "menu_qris"),
        ),
    )
...
func showPaymentOptions(bot *tgbotapi.BotAPI, chatID int64, days int) {
    var price string
    switch days {
    case 15:
        price = "6k"
    ...
    msgText := fmt.Sprintf("💳 *PEMBAYARAN AKUN %d HARI*\n\n" +
        "Harga: *%s*\n\n" +
    ...
    keyboard := tgbotapi.NewInlineKeyboardMarkup(
        tgbotapi.NewInlineKeyboardRow(
            tgbotapi.NewInlineKeyboardButtonData("💸 Bayar dengan QRIS", fmt.Sprintf("pay_qris:%d", days)),
        ),
        tgbotapi.NewInlineKeyboardRow(
...
func showQrisInfo(bot *tgbotapi.BotAPI, chatID int64) {
    qrisConfig, err := loadQrisConfig()
    if err != nil {
        sendMessage(bot, chatID, "❌ Gagal memuat konfigurasi QRIS.")
        return
    }

    msgText := fmt.Sprintf("💳 *INFORMASI PEMBAYARAN QRIS*\n\n" +
        "Merchant ID: `%s`\n\n" +
        ...
        "• 60 Hari: 24k\n" +
        "• 90 Hari: 35k\n\n" +
        "Setelah pembayaran, akun akan dibuat otomatis.",
        qrisConfig.MerchantID)
...
func initiateQrisPayment(bot *tgbotapi.BotAPI, chatID int64, days int) {
    qrisConfig, err := loadQrisConfig()
    if err != nil {
        sendMessage(bot, chatID, "❌ Gagal memuat konfigurasi QRIS.")
        return
    ...
    // Generate payment ID
    paymentID := generateRandomPassword(10)
    
    // Store payment info
    pendingPayments[paymentID] = &PaymentInfo{
    ...
    msgText := fmt.Sprintf("💳 *PEMBAYARAN QRIS - %d HARI*\n\n" +
        "Silakan scan QRIS di bawah ini untuk melakukan pembayaran sebesar *Rp %d*\n\n" +
        "ID Pembayaran: `%s`\n\n" +
        "⏰ Pembayaran akan kadaluarsa dalam 24 jam\n\n" +
        "Setelah pembayaran, akun akan dibuat otomatis.",
        days, price, paymentID)

    keyboard := tgbotapi.NewInlineKeyboardMarkup(
        tgbotapi.NewInlineKeyboardRow(
            tgbotapi.NewInlineKeyboardButtonData("🔄 Cek Status Pembayaran", fmt.Sprintf("check_payment:%s", paymentID)),
        ),
        tgbotapi.NewInlineKeyboardRow(
            tgbotapi.NewInlineKeyboardButtonData("⬅️ Kembali", "cancel"),
        ),
    ...
    // Create QR code image (using qrisConfig.DataQris)
    // Note: In a real implementation, you would generate a QR code image from the QRIS data
    // For now, we'll just display the QRIS data as text
    qrisText := fmt.Sprintf("```\n%s\n```", qrisConfig.DataQris)
...
func checkPaymentStatus(bot *tgbotapi.BotAPI, chatID int64, paymentID string) {
    ...
    if payment.Status == "paid" {
        // Create the account
        createUser(bot, chatID, payment.Username, payment.Days)
        
        // Remove from pending payments
        delete(pendingPayments, paymentID)
        
        msgText := fmt.Sprintf("✅ *PEMBAYARAN BERHASIL*\n\n" +
            "ID Pembayaran: `%s`\n" +
            "Akun telah dibuat dengan detail:\n" +
            "• Username: `%s`\n" +
            "• Durasi: %d hari",
            paymentID, payment.Username, payment.Days)

        msg := tgbotapi.NewMessage(chatID, msgText)
        msg.ParseMode = "Markdown"
        sendAndTrack(bot, msg)
    } else {
        msgText := fmt.Sprintf("⏳ *PEMBAYARAN PENDING*\n\n" +
            "ID Pembayaran: `%s`\n" +
            "Status: %s\n\n" +
...
func startPaymentChecker(bot *tgbotapi.BotAPI, adminID int64) {
    ...
    for range ticker.C {
        for paymentID, payment := range pendingPayments {
            ...
            // If payment is successful, create the account
            // This is just a placeholder for the actual payment checking logic
        }
    }
}
...
func loadQrisConfig() (QrisConfig, error) {
    ...
    // For now, we'll use the provided configuration
